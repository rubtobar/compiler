...
public class NodeCall extends Node {
    ...
    public void generateCode(VarTable vt, ProcTable pt, LabelCount lt, ThreeAddrCode gen) {
        if (pt.procTable.get(procId).returnSize > 0) {
            ...
            gen.add(Operator.RETURN_SPACE, null, null, "p" + procId);
        }
        if (contCall != null) {
            ArrayList<Integer> params = contCall.generateCode(vt, pt, lt, gen);
            for (int i = params.size() - 1; i >= 0; i--) {
                gen.add(Operator.PARAM, null, null, "v" + params.get(i));
            }
        }
        if (tid != null) {
            gen.add(Operator.CALL, "v" + tid, null, "p" + procId);
        } else {
            gen.add(Operator.CALL, null, null, "p" + procId);
        }
    }
